...
| POST `/sessions/{session_id}/steps/{step_id}/verifications` (two arms: `self_report` settles at once and answers 200 `SelfReported`; exactly one `evidence_ids` entry queues a `verify` Operation and answers 202 `Pending`, with the result on the event stream) | U or C | `{V,claim_id:uuid,evidence_ids:[uuid],self_report?:string<=1000}`; nonempty evidence OR self_report required | 202 Pending → Verification | I/AI/A; 422 `evidence_required`, 409 `evidence_stale` |
