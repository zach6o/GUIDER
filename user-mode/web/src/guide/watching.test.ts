...
function harness(options: {
  ...
  const api = {
    startWatching: vi.fn(async () => ({
      ...
      consent_version: 'observation-draft-1', session,
    })),
...
describe('switching watching on', () => {
  it('sends the exact notice version the user was shown', async () => {
    ...
    await context.watcher.start('observation-draft-1');
    expect(context.api.startWatching).toHaveBeenCalledWith(session, 'observation-draft-1');
  ...
  it('refuses to start without a session to watch for', async () => {
    ...
    await expect(watcher.start('observation-draft-1')).rejects.toThrow(/Start the guide/);
